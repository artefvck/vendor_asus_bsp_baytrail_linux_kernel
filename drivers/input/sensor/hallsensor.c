...
int hall_get_din()
{
	int result = intel_mid_pmic_readb(HALL_PMIC_CTLI)&0x01 ;
	return result ;
}
...
#else
			switch(board_id){	
			case ER:enable_irq(hall_sensor_dev->irq_pmic);break;
			case PR:
...
#else
			switch(board_id){	
			case ER:disable_irq(hall_sensor_dev->irq_pmic);break;
			case PR:
